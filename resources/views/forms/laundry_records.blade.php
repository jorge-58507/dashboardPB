...
<div class="bg-white p-6 rounded-lg shadow-lg">
    <div class="flex flex-col lg:flex-row justify-between items-center mb-6">
        ...
        <form id="laundry-filter-form" action="{{ route('formulario.laundry.records') }}" method="GET" onsubmit="event.preventDefault();" class="mt-4 lg:mt-0">
            ...
            <table class="min-w-full bg-white border border-gray-200">
                ...
                <tbody>
                    @foreach($records as $i => $record)
                        <tr id="row-laundry-{{ $record['laundry_id'] }}" class="{{ ($record['laundry_status'] === 0) ? 'bg-gray-700 text-white' : '' }}">
                            @foreach($displayHeaders as $colIndex => $headerName)
                                @switch($colIndex)
                                    @case('laundry_dateInit')
                                        <td class="py-2 px-4 border-b">{{date('d-m-Y', strtotime($record[$colIndex]))}}</td>
                                        @break
                                    @case('laundry_dateFinish')
                                        <td class="py-2 px-4 border-b">{{date('d-m-Y', strtotime($record[$colIndex]))}}</td>
                                        @break
                                    @case('laundry_status')
