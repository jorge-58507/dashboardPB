...
class FormController extends Controller
{
     ...
     */
    public function showGasForm()
    {
        //return view('forms.gas_consumption');
    }
    ...
    public function submitSales(Request $request)
    {
        // Validación de datos
        $validator = Validator::make($request->all(), [
            'montoTotalHospedaje'       => ['required', 'numeric', 'min:0'],
            'cantidadClienteHospedado'  => ['required', 'integer', 'min:0'],
            'totalVentaOtroIngreso'     => ['required', 'numeric', 'min:0'],
            'fechaRegistro'             => ['required', 'date', 'before_or_equal:today'], // La fecha no puede ser futura
        ], [
            'montoTotalHospedaje.required' => 'El campo Monto Total Ventas Hospedaje es obligatorio.',
            'montoTotalHospedaje.numeric' => 'El campo Monto Total Ventas Hospedaje debe ser un número.',
            'montoTotalHospedaje.min' => 'El campo Monto Total Ventas Hospedaje no puede ser negativo.',
            'cantidadClienteHospedado.required' => 'El campo Cantidad Clientes Hospedados es obligatorio.',
            'cantidadClienteHospedado.integer' => 'El campo Cantidad Clientes Hospedados debe ser un número entero.',
            'cantidadClienteHospedado.min' => 'El campo Cantidad Clientes Hospedados no puede ser negativo.',
            'totalVentaOtroIngreso.required' => 'El campo Total Ventas Otros Ingresos es obligatorio.',
            'totalVentaOtroIngreso.numeric' => 'El campo Total Ventas Otros Ingresos debe ser un número.',
            'totalVentaOtroIngreso.min' => 'El campo Total Ventas Otros Ingresos no puede ser negativo.',
        ...
        $rowData = [
            ...
            mt_rand($min, $max), // Columna 4 (índice 3)
            $userId,                       // Columna 5 (índice 4) - user_id
            mt_rand($min, $max), // Columna 6 (índice 5)
            mt_rand($min, $max), // Columna 7 (índice 6)
            mt_rand($min, $max), // Columna 8 (índice 7)
            $request->input('fechaRegistro'),     // Columna 9 (índice 8) - fechaRegistro
            ...
            mt_rand($min, $max), // Columna 14 (índice 13)
            floatval($request->input('montoTotalHospedaje')), // Columna 15 (índice 14) - montoTotalHospedaje
            ...
            floatval($request->input('cantidadClienteHospedado')), // Columna 19 (índice 18) - cantidadClienteHospedado
            mt_rand($min, $max), // Columna 20 (índice 19)
            floatval($request->input('totalVentaOtroIngreso')), // Columna 21 (índice 20) - totalVentaOtroIngreso
        ...
        try {
            ...
            if ($result->getUpdates() && $result->getUpdates()->getUpdatedRows() > 0) {
                Log::info('Datos de ventas guardados con éxito:', $request->all());
                return response()->json(['message' => 'Datos de ventas guardados con éxito.'], 200);
            } else {
                Log::error('Fallo al añadir fila de ventas a Google Sheet, no se actualizaron filas.', ['result' => $result]);
                return response()->json(['message' => 'Hubo un problema al guardar los datos de ventas.'], 500);
            }

        } catch (\Exception $e) {
            Log::error('Error al guardar datos de ventas: ' . $e->getMessage(), ['exception' => $e]);
            return response()->json(['message' => 'Error en el servidor al comunicarse con Google Sheets.', 'error' => $e->getMessage()], 500);
        }
    }
